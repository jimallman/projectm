...
MilkdropPresetFactory::allocate(const std::string& url, const std::string& name, const std::string& author)
{
    ...
    {
        //TODO: DEBUG("allocating? idle? preset using IDLE_PRESET_PROTOCOL... url='%s', name='%s', path='%s'", url.c_str(), name.c_str(), path.c_str());
        return IdlePresets::allocate(this, path, presetOutputs);
    }
    else
    {
        //TODO: DEBUG("returning new preset... url='%s', name='%s', path='%s'", url.c_str(), name.c_str(), path.c_str());
